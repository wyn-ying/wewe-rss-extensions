...
def register(feedid: str, cron_expr: str, cron_idx: int, rss: str = 'wewe') -> None:
    func = get_cron_func(feedid, rss)
    ...
    for feedid, crons in feedcron_conf.items():
        for cron_idx, cron_expr in enumerate(crons):
            if not croniter.is_valid(cron_expr):
                ...
                continue
            register(feedid, cron_expr, cron_idx, rss)
...
def add_cron():
    ...
    register(feedid, cron_expression, cron_idx, RSSTYPE)
